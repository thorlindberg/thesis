\documentclass[10pt, twocolumn, letterpaper]{article}

%% Packages

\usepackage[utf8x]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[a4paper, margin=2.5cm]{geometry}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage[colorinlistoftodos]{todonotes}
\usepackage[colorlinks=true, allcolors=black]{hyperref}
\usepackage{natbib}
\usepackage{subcaption}
\usepackage{subfiles}
\usepackage{titlesec}
\usepackage[english]{babel}
\usepackage{csvsimple}

%% Customisation

\graphicspath{{figures/}}
\bibliographystyle{apalike}
\titleformat*{\section}{\centering\small}
\titleformat*{\subsection}{\textit\small}
\renewcommand{\thesection}{\Roman{section}} 
\renewcommand{\thesubsection}{\alph{subsection}}
\renewcommand{\bibsection}{\null \\ \centerline{\small{REFERENCES}}}
\newcommand{\charactercount}[1]{\immediate\write18{expr `texcount -1 -sum -merge #1.tex` + `texcount -1 -sum -merge -char #1.tex` - 1 > chars.txt}\input{chars.txt}characters}
\raggedbottom

%% Title

\title{
    \includegraphics[height=0.75cm]{logo.jpg} \\
    \vspace{1.25cm}
    \Large \textbf{Mapping Distributed Software Architectures for}  \\
    \Large \textbf{Plaintext and Binary Object Serialisation Formats}  \\
    % \LARGE \textbf{Human-Readability vs Type Safety in  \\
    % Plaintext and Binary Object Serialisation} \\
    % \Large \textbf{Architectures for Plaintext and Binary Object Serialisation} \\
}
\usepackage{authblk}
\author{Thor Wessel Lindberg (17858, mawl@itu.dk)}
\date{\charactercount{report} (incl. spaces)}

%% Content

\begin{document}

\maketitle

\subfile{sections/preface}

\section{INTRODUCTION}
\subfile{sections/introduction}

\section{DISTRIBUTED COMPUTING} % layers, OSI model
\subfile{sections/distributed}

\section{DATA TRANSMISSION} % REST APIs
\subfile{sections/transmission}

\section{SERIALISATION FORMATS} % plaintext and binary
\subfile{sections/serialisation}

\section{SOFTWARE ARCHITECTURE} % backend, frontend, distribution, developers
\subfile{sections/architecture}

\section{IMPLEMENTATION CASE} % shortcut
\subfile{sections/implementation}

\section{RELATED WORK} % object serialisation, format/performance/size comparison
\subfile{sections/relatedwork}

\section{HUMAN-READABILITY OR TYPE SAFETY} % validity and type safety
\subfile{sections/comparison}

\section{DISCUSSION}
\subfile{sections/discussion}

\section{CONCLUSION}
\subfile{sections/conclusion}

\section{FUTURE WORK}
\subfile{sections/futurework}

\bibliography{bibliography}

\end{document}
